...
public class ApplicationCache {
  ...
  public ApplicationCache(AttributeService attributeService) {
    attributeStatusCache =
        cacheDaily(id -> attributeService.getAttributeStatusService().findById(id));
  ...
  public Optional<AttributeStatus> getAttributeStatus(String id) {
    return attributeStatusCache.getUnchecked(id);
  }

  public Optional<AppLanguage> getAppLanguage(String id) {
    return appLanguageCache.getUnchecked(id);
  }

  public Optional<Gender> getGender(String id) {
    return genderCache.getUnchecked(id);
  }

  public Optional<Permission> getPermission(String id) {
    return permissionCache.getUnchecked(id);
  }

  public Optional<Role> getRole(String id) {
    return roleCache.getUnchecked(id);
  }
  ...
  private static <K, V> LoadingCache<K, V> cache(
      Supplier<CacheBuilder<Object, Object>> cacheBuilder, Resolver<K, V> resolver) {
    return cacheBuilder
        .get()
        .build(
            new CacheLoader<K, V>() {
